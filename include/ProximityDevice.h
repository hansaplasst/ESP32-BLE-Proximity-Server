...
struct ProximityData {
  ...
  bool is_blocked = false;                      // Blocked status: true if the device is blocked, false otherwise
  bool is_admin = false;                        // Admin status: true if the device is an admin, false otherwise
  int8_t rssi_threshold = -100;                 // Proximity (RSSI) value. On bonding the rssi_threshold of the client will be set.
  int16_t mom_switch_delay = 300;               // Delay in milliseconds for the momOpen / momClose command. Default is 300ms. Min 10ms, max 30000ms.
  std::string rssi_command = "momOpen";         // Command to execute if device is in proximity. RSSI >= rssi_threshold. If empty, no command is executed.
